...
const editBookPost = [
  ...
  body("author").trim().notEmpty().withMessage("Author is required.").isLength({ max: 255 }).withMessage("Author must be at most 255 characters long."),
  body("published_year").optional({ checkFalsy: true }).isInt({ min: 0 }).withMessage("Published year must be a valid year."),
  body("isbn").trim().optional({ checkFalsy: true }).isLength({ max: 20 }).withMessage("ISBN must be at most 20 characters long."),
  body("description").trim().optional({ checkFalsy: true }),
  body("pages").optional({ checkFalsy: true }).isInt({ min: 1 }).withMessage("Pages must be a positive number."),
  body("stock").optional({ checkFalsy: true }).isInt({ min: 0 }).withMessage("Stock must be a non-negative number."),
  body("image_url").trim().optional({ checkFalsy: true }).isURL().withMessage("Image URL must be a valid URL."),
  body("admin_password")
    .trim()
    .notEmpty()
